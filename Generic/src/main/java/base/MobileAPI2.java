...
public class MobileAPI2 {
    ...
    @BeforeMethod
    public static AppiumDriver<WebElement> getDriver(String OS, String deviceName, String version) throws IOException {
        platform = OS;
        ...
        if (OS.equalsIgnoreCase("Android")) {
            ...
            capabilities.setCapability(MobileCapabilityType.APP, "D:\\java\\MobileAutomation\\booking\\src\\appAPK\\com.thomsonreuters.reuters_3.4.2-342_minAPI16(arm64-v8a,armeabi,armeabi-v7a,mips,mips64,x86,x86_64)(nodpi)_apkmirror.com.apk");
            appiumDriver = new AndroidDriver<WebElement>(new URL("http://localhost:4723/wd/hub"), capabilities);

        } else {
            cap.setCapability(MobileCapabilityType.PLATFORM_VERSION, version);
            cap.setCapability(MobileCapabilityType.APP_PACKAGE, "com.booking");
            cap.setCapability(MobileCapabilityType.AUTOMATION_NAME, "XCUITest");
            cap.setCapability(MobileCapabilityType.PLATFORM_NAME, MobilePlatform.IOS);
            cap.setCapability(MobileCapabilityType.APP_PACKAGE, "com.booking");
            if (appiumDriver == null) {
                appiumDriver = new IOSDriver<WebElement>(new URL("http://localhost:4723/wd/hub"), cap);
            }
        }
    ...
    public static void functionSwipe(String Direction, int swipeTime, int Offset) {
        ...
        if (Direction.equalsIgnoreCase("Up")) {
            ((appiumDriver)).swipe(startx / 2, starty - Offset, startx / 2, endy, swipeTime);
        } else if (Direction.equalsIgnoreCase("Down")) {
            ((appiumDriver)).swipe(startx / 2, endy + Offset, startx / 2, starty, swipeTime);
        } else if (Direction.equalsIgnoreCase("Right")) {
            starty = size.height / 2;
            endy = size.height / 2;
            startx = (int) (size.width * 0.10);
            int endx = (int) (size.width * 0.90);
            ((appiumDriver)).swipe(startx + Offset, starty, endx, endy, swipeTime);
    ...
    public void scrollToElement(AppiumDriver<MobileElement> driver, String element) {
        MobileElement we = driver.findElementByXPath(element);
